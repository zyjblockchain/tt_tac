...
func GetAddressTokenTransfers(contractaddress, address string, page, offset int) ([]etherscan.ERC20Transfer, error) {
	...
	url := fmt.Sprintf("https://rinkeby.etherscan.io/api?module=account&action=tokentx&contractaddress=%s&address=%s&page=%d&offset=%d&sort=desc&apikey=%s", contractaddress, address, page, offset, apiKey)
